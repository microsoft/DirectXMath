...
namespace DirectX
{
    ...
    float* XM_CALLCONV XMSHRotate(_Out_writes_(order* order) float* result,
        ...
        _In_reads_(order* order) const float*                       input) noexcept;
    ...
    float* XMSHMultiply(_Out_writes_(order* order) float* result,
        ...
        _In_reads_(order* order) const float*             inputG) noexcept;
    ...
    bool XM_CALLCONV XMSHEvalDirectionalLight(_In_ size_t order,
        ...
        _Out_writes_opt_(order* order) float*             resultB) noexcept;
    ...
    HRESULT SHProjectCubeMap(_In_ size_t      order,
        _In_ const D3D12_RESOURCE_DESC&       desc,
        _In_ const D3D12_SUBRESOURCE_DATA     cubeMap[6],
        _Out_writes_opt_(order* order) float* resultR,
        _Out_writes_opt_(order* order) float* resultG,
        _Out_writes_opt_(order* order) float* resultB) noexcept;
#endif
